...
def generate_constellation(width, height, viewport_h, filename):
    print(f"Generating the constellation in high definition...")
    
    # 🌟 SUPERSAMPLING: We render at 2x size so we can smooth it afterward
    render_w = width * 2
    render_h = height * 2
    ...
    # 1. Draw the background clouds (2x scale)
    ...
    # 🌟 THE SMOOTHING SECRET: We resize the image to its actual size using the LANCZOS filter
    print(f"Applying anti-aliasing (LANCZOS)...")
    img_smooth = img.resize((width, height), Image.Resampling.LANCZOS)

    # Saving in high-quality WebP format
    final_img = img_smooth.convert("RGB")
    final_img.save(filename, "WEBP", quality=95) # Boosted quality to 95% to avoid artifacts
    print(f"✓ File {filename} generated (Pixel-Perfect Quality)!\n")

if __name__ == "__main__":
    # 1. Desktop version (1920x10000)
